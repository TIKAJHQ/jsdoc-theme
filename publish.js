...
exports.publish = function (taffyData, opts, tutorials) {
    ...
    view.projectTitle = opts.title || "Documentation";
    ...
    let readmeContent ="";
    let packagePath =path.getResourcePath("package.json");
    if(packagePath) {
        try {
            let p = JSON.parse(fs.readFileSync(packagePath, "utf-8"));
            readmeContent = "<h4> "+p.name+" <i style='float:right;'> "+p.version+"</i> </h4><br/><p>"+p.description+"</p><br><br/>"
        }catch(e){
        }
    }
    ...
    // Generate documentation in single page.
    // let topLevelClasses = []
    //     .concat(helper.find(namespaces, {}))
    //     .concat(helper.find(modules, {}))
    //     .concat(helper.find(classes, {}))
    // helper.longnameToUrl.api = "api.html"
    // generate('api', "Documentation", topLevelClasses,helper.longnameToUrl.api );
